import KeccakP
import XCTest

final class KeccakPTests: XCTestCase {
    func testPermutation() {
        var state = State()
        
        state.permute(rounds: 24)
        
        XCTAssert(state.elementsEqual([
            ,
        ]))
        
        state.permute(rounds: 24)
        
        XCTAssert(state.elementsEqual([
            ,
        ]))
    }
    
    func testMRAC() {
        var state = State()
        
        XCTAssertEqual(state.count, 200)
        XCTAssertEqual(state.indices, 0..<200)
        
        XCTAssertEqual(state.index(after: state.startIndex), 1)
        XCTAssertEqual(state.index(before: state.endIndex), 199)
        
        state[101] = 0xff
        XCTAssertEqual(state[101], 0xff)
        
        state.withContiguousMutableStorageIfAvailable {
            for (index, element): (_, UInt8) in zip($0.indices, 0...) {
                $0[index] = element
            }
        }
        state.withContiguousStorageIfAvailable {
            XCTAssert($0.elementsEqual(0..<200))
        }
        
        state.withUnsafeMutableBytes {
            $0.copyBytes(from: 1...200)
        }
        state.withUnsafeBytes {
            XCTAssert($0.elementsEqual(1...200))
        }
        
        XCTAssert(state.elementsEqual(1...200))
    }
}
